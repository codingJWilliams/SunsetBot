...
class SrCommand extends Command {
    ...
    async exec(message, args) {
      ...
      if (args.op === "give") {
        var possible = await global.mongo.collection("sharableroles").findOne({role: args.sr.id, owner: message.author.id});
        ...
        if (!args.recip) return message.channel.send(
          new util.d.RichEmbed()
          .setTitle(":octagonal_sign: I could not do that")
          .setColor(0xfd5c5c)
        ...
        message.channel.send(new util.d.RichEmbed().setTitle(":white_check_mark: Role " + args.sr.name + " given").setColor(0x49bd1a).setFooter(util.makeFooter(message.author), message.author.displayAvatarURL))
      } 
      if (args.op === "take") {
        var possible = await global.mongo.collection("sharableroles").findOne({role: args.sr.id, owner: message.author.id});
        if (!possible) return message.channel.send(
        ...
        //var fing = await global.mongo.collection("sharableroles").find({}).toArray();
        message.channel.send(new util.d.RichEmbed().setTitle(":white_check_mark: Role " + args.sr.name + " taken").setColor(0x49bd1a).setFooter(util.makeFooter(message.author), message.author.displayAvatarURL))
      } 
    }
}
