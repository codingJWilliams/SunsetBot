...
class SrCommand extends Command {
    ...
    async exec(message, args) {
      ...
      if (args.op === "add") {
        ...
        await global.mongo.collection("sharableroles").findOneAndDelete({role: args.sr.id})
        message.channel.send(new util.d.RichEmbed().setTitle(":white_check_mark: Role " + args.sr.name + " is no longer marked as sharable").setColor(0x49bd1a).setFooter(util.makeFooter(message.author), message.author.displayAvatarURL))
      }
    }
}
